...
# broad transaction terms, plus Google's when: operator so Google filters
# for recency server-side (relevance-sorted results otherwise bury recent
# articles behind older, better-matching ones).
...
def fetch_news():
    ...
    for query in SEARCH_QUERIES:
        ...
        if status != 200 or (n_entries == 0 and feed.get("bozo")):
            print(
                f"  [diagnostic] query={query!r} http_status={status} "
                f"entries={n_entries} bozo={feed.get('bozo')} "
    ...
    print(
        f"Raw RSS entries across all queries (before date filtering): "
        f"{total_raw_entries}. Failed queries: {failed_queries}/{len(SEARCH_QUERIES)}."
    )
    if all_published_dates:
        newest = max(all_published_dates)
        oldest = min(all_published_dates)
        print(
            f"Publish date range in raw results: oldest={oldest.isoformat()}, "
            f"newest={newest.isoformat()}. Cutoff (now - {LOOKBACK_HOURS}h)={cutoff.isoformat()}."
        )
...
def classify_batch(articles):
    ...
    Returns (deals: list, had_error: bool)."""
...
{{
  ...
  "sector": "...",
  "deal_value": "e.g. Rs 245 Crore or 'Undisclosed'",
    ...
    for attempt in range(max_retries):
        try:
            ...
            if resp.status_code == 429:
                wait = 20 * (attempt + 1)
                print(
                    f"Rate limited (429) on a batch of {len(articles)} articles "
...
def _load_logo_base64():
    """Loads the Prowess Advisors logo from logo.png (if present next to this
    script) and returns it base64-encoded for inline embedding in the email.
    Returns None if the logo file isn't found, in which case the email header
    is rendered without a logo."""
...
PROWESS_LOGO_BASE64 = _load_logo_base64()
...
def send_email(deals):
    ...
    logo_html = ""
    if PROWESS_LOGO_BASE64:
        logo_html = (
            f'<img src="data:image/png;base64,{PROWESS_LOGO_BASE64}" '
            f'alt="Prowess Advisors" style="height:52px;width:auto;display:inline-block;" />'
        )
...
def main():
    ...
    for batch_num, batch in enumerate(batches, start=1):
        ...
        print(
            f"Batch {batch_num}/{len(batches)}: {len(batch)} articles -> "
            f"{len(deals)} deal(s) found. Error: {had_error}"
        )
